...
namespace kiko
{
	...
	public:
		...
		Matrix33() = default;
		Matrix33(const vec3& row1, const vec3& row2, const vec3& row3) //identity matrix
		{
			rows[0] = row1;
			rows[1] = row2;
			rows[2] = row3;
	...
	{
		// | a b c |   | j k l |  aj + bm + cp, ak + bn + cq,    | a b | * | e f | ae + bg, af + bh
		// | d e f | * | m n o |  dj + em + fp, dk + en + fq     | c d |   | g h | ce + dg, cf + dh
		// | g h i |   | p q r |  gj + hm + ip, 
	...
	{
		return std::atan2( rows[1][0], rows[0][0] ); ///is this right?
	}

	inline vec2 Matrix33::GetScale() const
	{
		vec2 x = { rows[0][0], rows[1][1]};
		vec2 y = { rows[0][0], rows[1][1]};
