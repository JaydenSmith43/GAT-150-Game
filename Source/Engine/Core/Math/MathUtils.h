#pragma once
#include <cmath>
//#include <utility> //if swap wasn't working
...
namespace kiko
{
	...
	template<typename T>
	constexpr T Lerp(const T& a, const T& b, float t) // t = 0 <-> 1, 0 is point A and 1 is point B
	{
		return (a * (1.0f - t)) + (b * t); //move between two values
	}
}